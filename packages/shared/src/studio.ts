...
export interface StudioResult { remappings?: string[]; revision: number; compilerVersion: string; sources: Record<string, string>; program: StudioProgram | null; diagnostics: StudioDiagnostic[] }
...
export interface StudioRequest { remappings?: string[]; revision: number; sources: Record<string, string>; edit?: StudioEdit }
export interface StudioDraft { title: string; edit: StudioEdit; text: string; template?: string; fields?: Record<string, string> }
export interface StudioWorkspace { remappings?: string[]; pendingEdit?: StudioDraft; schemaVersion: 1; sources: Record<string, string>; positions: Record<string, { x: number; y: number }> }
...
export function parseStudioWorkspace(value: unknown): StudioWorkspace {
  ...
  validateStudioRemappings(data.remappings);
  ...
  return { schemaVersion: 1, sources: data.sources, ...(data.remappings ? { remappings: data.remappings } : {}), positions, ...(draft ? { pendingEdit: draft } : {}) };
}
...
export function validateStudioRemappings(value: unknown): asserts value is string[] | undefined {
  if (value === undefined) return;
  if (!Array.isArray(value) || value.length > 100 || value.some(line => typeof line !== "string" || line.length > 1000 || !/^[^=\s]+=[^=\s]+$/.test(line))) throw new Error("Import remappings must be prefix=path entries (at most 100).");
}
