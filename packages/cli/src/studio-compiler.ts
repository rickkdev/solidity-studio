...
import type { StudioDiagnostic, StudioEdit, StudioNode, StudioProgram, StudioRequest, StudioResult, StudioSpan, StudioBuild, StudioAbiInput, StudioCallable } from "@codevis/shared";
...
function validateSources(sources: unknown): asserts sources is Record<string, string> {
  ...
  if (entries.reduce((sum, [, source]) => sum + Buffer.byteLength(source as string), 0) > 2_000_000) throw new Error("Workspace exceeds the 2 MB source limit.");
}

export function analyzeStudio(request: StudioRequest): StudioResult {
...
function charOffset(source: string, bytes: number): number { return Buffer.from(source).subarray(0, bytes).toString("utf8").length; }

export function generateStudio(request: StudioRequest): StudioResult {
...
export function buildStudioRuntime(request: StudioRequest): StudioBuild {
